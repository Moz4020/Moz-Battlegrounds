...
export class SendSwapTeamIntentEvent implements GameEvent {
  constructor(
    public readonly targetPlayer: string,
    public readonly targetTeam: string,
  ) {}
}

export class SendResetTeamAssignmentsIntentEvent implements GameEvent {}
...
export class Transport {
  ...
  constructor(
    ...
    this.eventBus.on(SendSwapTeamIntentEvent, (e) =>
      this.onSendSwapTeamIntent(e),
    );

    this.eventBus.on(SendResetTeamAssignmentsIntentEvent, () =>
      this.onSendResetTeamAssignmentsIntent(),
    );
  }
  ...
  private connectRemote(
    ...
    this.socket.onclose = (event: CloseEvent) => {
      ...
      if (event.code === 1002) {
        showAlert(`connection refused: ${event.reason}`, "error");
  ...
  private onSendSwapTeamIntent(event: SendSwapTeamIntentEvent) {
    this.sendIntent({
      type: "swap_team",
      clientID: this.lobbyConfig.clientID,
  ...
  private onSendResetTeamAssignmentsIntent() {
    this.sendIntent({
      type: "reset_team_assignments",
      clientID: this.lobbyConfig.clientID,
    });
  }

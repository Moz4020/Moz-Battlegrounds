...
export class GameServer {
  ...
  // Manual team assignments made by the lobby owner (clientID -> team name)
  private manualTeamAssignments: Map<ClientID, string> = new Map();
        ...
        switch (clientMsg.type) {
          ...
          case "intent": {
            ...
            switch (clientMsg.intent.type) {
              ...
              // Handle swap_team intent via WebSocket (lobby only)
              case "swap_team": {
                const swapIntent = clientMsg.intent;
                // Only allow in lobby phase
                if (this._hasStarted) {
                  this.log.warn(`Cannot swap teams after game started`, {
                    clientID: client.clientID,
                  });
                  return;
                }
                ...
                // Check if the authenticated client is the lobby creator
                if (client.clientID !== this.lobbyCreatorID) {
                  this.log.warn(`Only lobby creator can swap teams`, {
                    clientID: client.clientID,
                    creatorID: this.lobbyCreatorID,
                    targetPlayer: swapIntent.targetPlayer,
                    targetTeam: swapIntent.targetTeam,
                    gameID: this.id,
                  });
                  return;
                }

                // Validate target player exists in lobby
                const targetClient = this.activeClients.find(
                  (c) => c.clientID === swapIntent.targetPlayer,
                );
                if (!targetClient) {
                  this.log.warn(`Target player not found in lobby`, {
                    targetPlayer: swapIntent.targetPlayer,
                    gameID: this.id,
                  });
                  return;
                ...
                // Store the manual team assignment
                this.manualTeamAssignments.set(
                  swapIntent.targetPlayer,
                  swapIntent.targetTeam,
                );

                // Update gameConfig with the new assignments
                this.gameConfig.manualTeamAssignments = Object.fromEntries(
                  this.manualTeamAssignments,
                );

                this.log.info(`Team assignment updated`, {
                  creatorID: client.clientID,
                  targetPlayer: swapIntent.targetPlayer,
                  targetTeam: swapIntent.targetTeam,
                  gameID: this.id,
              ...
              // Handle reset_team_assignments intent via WebSocket (lobby only)
              case "reset_team_assignments": {
                // Only allow in lobby phase
                if (this._hasStarted) {
                  this.log.warn(
                    `Cannot reset team assignments after game started`,
                    {
                      clientID: client.clientID,
                    },
                  );
                ...
                if (client.clientID !== this.lobbyCreatorID) {
                  this.log.warn(
                    `Only lobby creator can reset team assignments`,
                    {
                      clientID: client.clientID,
                      creatorID: this.lobbyCreatorID,
                      gameID: this.id,
                ...
                this.manualTeamAssignments.clear();
                this.gameConfig.manualTeamAssignments = undefined;

                this.log.info(`Team assignments reset to auto`, {
                  creatorID: client.clientID,
                  gameID: this.id,
                });
                return;
  ...
  public gameInfo(): GameInfo {
    return {
      ...
      clients: this.activeClients.map((c) => ({
        username: c.username,
        clientID: c.clientID,
        assignedTeam: this.manualTeamAssignments.get(c.clientID),
      })),

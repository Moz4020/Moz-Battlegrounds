...
export type SwapTeamIntent = z.infer<typeof SwapTeamIntentSchema>;
export type ResetTeamAssignmentsIntent = z.infer<
  typeof ResetTeamAssignmentsIntentSchema
>;
...
export interface ClientInfo {
  clientID: ClientID;
  username: string;
  assignedTeam?: string; // Manually assigned team by lobby owner
}
...
// Manual team assignments made by lobby owner (clientID -> team name)
export const ManualTeamAssignmentsSchema = z.record(z.string(), TeamSchema);
export type ManualTeamAssignments = z.infer<typeof ManualTeamAssignmentsSchema>;
...
export const GameConfigSchema = z.object({
  ...
  freeNukes: z.boolean(),
  permanentAllies: z.boolean(),
  maxPlayers: z.number().optional(),
  maxTimerValue: z.number().int().min(1).max(120).optional(),
  disabledUnits: z.enum(UnitType).array().optional(),
  playerTeams: TeamCountConfigSchema.optional(),
  manualTeamAssignments: ManualTeamAssignmentsSchema.optional(),
});
...
export const SwapTeamIntentSchema = BaseIntentSchema.extend({
  type: z.literal("swap_team"),
  targetPlayer: ID,
  targetTeam: TeamSchema,
});

export const ResetTeamAssignmentsIntentSchema = BaseIntentSchema.extend({
  type: z.literal("reset_team_assignments"),
});
...
const IntentSchema = z.discriminatedUnion("type", [
  ...
  SwapTeamIntentSchema,
  ResetTeamAssignmentsIntentSchema,
]);
